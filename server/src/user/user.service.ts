...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto): Promise<User | null> {
    try {
      ...
      if (!user) {
          throw new HttpException('Не удалось создать пользоваетля!', HttpStatus.BAD_REQUEST);
      }
      return user;
    } catch (error) {
      console.log(error);
  ...
  async findAll(): Promise<User[] | null> {
    try {
      ...
      if (!users) {
          throw new HttpException('Не удалось получить пользователей из DB!', HttpStatus.BAD_REQUEST);
      }
      return users;
    } catch (error) {
      console.log(error);
  ...
  async findOne(userWhereUniqueInput: Prisma.UserWhereUniqueInput): Promise<User | null> {
    try {
      ...
      if (!user) {
          throw new HttpException('Не удалось получить пользователя из DB!', HttpStatus.BAD_REQUEST);
      }
      return user;
    } catch (error) {
      console.log(error);
